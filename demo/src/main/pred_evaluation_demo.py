...
def rangeto1(scores_d):
    ...
    for lbl, scores in scores_d.items():
        ...
        sim_sc_norm = [float((x-mn)/(mx-mn)) for x in scores]
...
def decompose(lkkm_d):
    ...
    for lbl, lkkm in lkkm_d.items():
        ...
        if len(lkkm) > 10:
            ...
            score_frame.fillna(max(lkkm), inplace=True)
...
def st_demo(combine_d, scores_d, sims_d, lkkm_d, anno_sgm_d, res_d, cm_disp, roc_disp):
    ...
    * Normal
    ...
    for vid in sorted(os.listdir(VIDEO_PATH)):
        # print("vid ---> ", vid)
        cnt += 1
        if vid.startswith("Explosion011_x264.mp4"):
            ...
            with exp:
                ...
                st.checkbox("Display CRAFT", value=True, key="{}{}FLOW".format(cat, num))
                st.checkbox("Display LKKM", value=True, key="{}{}LKKM".format(cat, num))
...
def main(pred_path, sim_path, delta_path, lkkm_path):
    ...
    #LKKM - consider relative scale    
    lkkm_d = decompose(lkkm_d)
    lkkm_d = relative_scale(lkkm_d, 20, 1.6)
    lkkm_d = center_on_avg_err(lkkm_d)
    lkkm_d = rangeto1(lkkm_d)
    
    # combine_d, sims_d, scores_d = score_combine(scores_d, sims_d, deltas_d)

    #DOD2: for display of CRAFT/sims_d potential -> uncomment combine_d above and send sims_d in for combine_d in evaluation and st_demo
    sims_d = individual_scale(sims_d, 1.4)
...
if __name__ == '__main__':
    ...
    lkkm_path = sys.argv[4]
    
    main(pred_path, sim_path, delta_path, lkkm_path)
